...
class AgentToolDefsMixin:
    ...
    def _all_tool_definitions(self) -> list[dict]:
        ...
        return [
            ...
            {"type": "function", "function": {"name": "experience_lookup", "description": "Look up past proven workflows for a similar complex task. Call when you want to check if there is a battle-tested pattern from past work (struggle-discovered, failure→success cycles). Only useful for complex multi-step coding tasks.", "parameters": {"type": "object", "properties": {"task_description": {"type": "string", "description": "Brief description of the task you are about to perform"}}, "required": ["task_description"]}}},
            # ── Task planning (OpenCode/Claude Code pattern) ─────────────
            {"type": "function", "function": {"name": "todo_write", "description": "Write or update the task plan. Use at start of multi-step tasks (3+ steps). Update status as work progresses.", "parameters": {"type": "object", "properties": {"todos_json": {"type": "string", "description": "JSON array of todos with id, content, status (pending/in_progress/completed/cancelled), priority (high/medium/low)"}}, "required": ["todos_json"]}}},
            {"type": "function", "function": {"name": "todo_read", "description": "Read the current task plan and check progress. Use before declaring work complete.", "parameters": {"type": "object", "properties": {}}}},
            {"type": "function", "function": {"name": "task_complete", "description": "Signal that the current task is fully complete. ONLY call when ALL todo items are done and work is verified. This is the ONLY way to exit autonomous execution mode.", "parameters": {"type": "object", "properties": {"result": {"type": "string", "description": "Clear summary of what was accomplished"}, "command": {"type": "string", "description": "Optional verification command (e.g. 'cargo test', 'npm test')"}}, "required": ["result"]}}},
            {"type": "function", "function": {"name": "give_up", "description": "Withdraw from a task that cannot be completed. Use when requirements are impossible, permissions missing, or root cause is unclear after multiple attempts.", "parameters": {"type": "object", "properties": {"reason": {"type": "string"}, "what_was_tried": {"type": "string"}, "suggestion": {"type": "string"}}, "required": ["reason", "what_was_tried"]}}},
