...
fn global_skills_dir() -> PathBuf {
    dirs::home_dir().unwrap_or_default().join(".evocli").join("skills")
}
...
pub async fn run(action: SkillAction) -> Result<()> {
    match action {
        ...
        SkillAction::Run { skill_id, dry_run } => {
            ...
            println!("Tip: run 'evocli' and use '/skill run {}' inside the TUI.", skill_id);
            Ok(())
        }
        SkillAction::Reload => {
...
fn cmd_export(skill_id: &str, output: Option<&str>) -> Result<()> {
    ...
    println!("   Share this file and use: evocli skill import {}", dst.display());
    Ok(())
}
...
fn cmd_import(path: &str, global: bool, force: bool) -> Result<()> {
    ...
    let skill_id = parsed
        .get("skill").and_then(|s| s.get("id")).and_then(|v| v.as_str())
        .unwrap_or_else(|| src.file_stem().and_then(|s| s.to_str()).unwrap_or("unknown"));
    ...
    if dst.exists() && !force {
        anyhow::bail!(
            ...
            skill_id, dst.display()
        );
    }

    std::fs::copy(&src, &dst)
        .with_context(|| format!("Failed to copy to {}", dst.display()))?;
...
fn cmd_show(skill_id: &str) -> Result<()> {
    ...
    if let Some(skill) = parsed.get("skill") {
        if let Some(name)    = skill.get("name").and_then(|v| v.as_str())    { println!("  Name:  {}", name); }
        if let Some(ver)     = skill.get("version").and_then(|v| v.as_str()) { println!("  Ver:   {}", ver); }
        if let Some(status)  = skill.get("status").and_then(|v| v.as_str())  { println!("  Status:{}", status); }
        if let Some(steps)   = skill.get("steps").and_then(|v| v.as_array()) {
            ...
            for (i, step) in steps.iter().enumerate() {
                ...
                let req    = step.get("requires_approval").and_then(|v| v.as_bool()).unwrap_or(false);
                println!("    {}. {} → {}{}", i + 1, id, action, if req { "  ⚠ requires approval" } else { "" });
            }
        }
    }
    println!();
...
fn find_skill_file(skill_id: &str) -> Result<PathBuf> {
    let candidates = [
        PathBuf::from(".evocli").join("skills").join(format!("{}.toml", skill_id)),
    ...
    candidates.iter()
        ...
        .cloned()
        .with_context(|| format!(
            "Skill '{}' not found in .evocli/skills/ or ~/.evocli/skills/", skill_id
        ))
}
...
fn read_skill_meta(path: &PathBuf) -> (Option<String>, Option<String>) {
    ...
    let name   = parsed.get("skill").and_then(|s| s.get("name")).and_then(|v| v.as_str()).map(str::to_string);
    let status = parsed.get("skill").and_then(|s| s.get("status")).and_then(|v| v.as_str()).map(str::to_string);
    (name, status)
}
