...
// ── Spinner ──────────────────────────────────────────────────────────────────
const SPINNERS: &[&str] = &["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"];
fn spinner_frame(tick: u8) -> &'static str { SPINNERS[(tick as usize) % SPINNERS.len()] }

// ── Layout mode ──────────────────────────────────────────────────────────────
#[derive(Debug, Clone, Copy, PartialEq)]
enum LayoutMode { Wide, Normal, Compact, Tiny }
impl LayoutMode {
    fn from_width(w: u16) -> Self {
        match w { w if w >= 120 => Self::Wide, w if w >= 60 => Self::Normal,
                  w if w >= 40 => Self::Compact, _ => Self::Tiny }
    }
}
...
fn input_area_height(h: u16) -> u16 {
    if h >= 40 { 4 } else if h >= 25 { 3 } else { 3 }
}
...
pub fn draw(f: &mut Frame, app: &mut App, textarea: &TextArea<'_>) {
    ...
    if area.width < 40 || area.height < 10 {
        draw_tiny_mode(f, app, area); return;
    }
    ...
    let constraints = if app.is_skill_running() {
        vec![Constraint::Length(2), Constraint::Min(6), Constraint::Length(3),
             notif_row, Constraint::Length(input_h), Constraint::Length(1)]
    } else {
        vec![Constraint::Length(2), Constraint::Min(6),
             notif_row, Constraint::Length(input_h), Constraint::Length(1)]
    };
...
fn draw_tiny_mode(f: &mut Frame, app: &App, area: Rect) {
    let s = match &app.state { AppState::Idle => "Ready", AppState::Thinking => "…",
        AppState::Streaming{..} => "▸", _ => "⚙" };
    let p = Paragraph::new(vec![
        Line::from(vec![
            Span::styled("◆ ", Style::default().fg(C_PURPLE).add_modifier(Modifier::BOLD)),
            Span::styled("EvoCLI ", Style::default().fg(USER_ACCENT).add_modifier(Modifier::BOLD)),
            Span::styled(s, Style::default().fg(C_YELLOW)),
        ]),
        Line::from(Span::styled("Terminal too small  (need ≥40×10)", Style::default().fg(FG_DIM))),
...
fn draw_title_bar(f: &mut Frame, app: &App, area: Rect, mode: LayoutMode) {
    ...
    let line = match mode {
        LayoutMode::Wide | LayoutMode::Normal => {
            ...
            let fixed_len = 3 + 6 + 2 + model.chars().count() + 4
                + 2 + tok_bar.chars().count() + cost_str.chars().count() + 4;
            ...
            let dir = if app.project_dir.chars().count() <= dir_budget {
                ...
                format!("…{}", safe_tail(&app.project_dir, dir_budget.saturating_sub(1)))
            };
...
fn token_bar(used: usize, max_ctx: usize, bar_w: usize) -> (String, Color) {
    ...
    let color = if pct >= 95 { C_RED }
        else if pct >= 80   { C_ORANGE }
...
fn draw_suggestions_popup(f: &mut Frame, app: &App, input_area: Rect) {
    ...
    if count == 0 { return; }
    let popup_h = count + 2;
    let popup_y = if input_area.y >= popup_h { input_area.y - popup_h } else { 0 };
    ...
    let items: Vec<ListItem> = app.suggestions.iter().enumerate().map(|(i, (cmd, desc))| {
        let sel = i == app.suggestion_idx;
        let c = safe_prefix(cmd, cmd_w);
        let content = if desc_w > 4 {
            format!("  {:<w$}  {}", c, safe_prefix(desc, desc_w), w = cmd_w)
        } else { format!("  {c}") };
        if sel { ListItem::new(content).style(Style::default().fg(BG_BASE).bg(USER_ACCENT).add_modifier(Modifier::BOLD)) }
        else   { ListItem::new(content).style(Style::default().fg(FG_TEXT).bg(BG_SURFACE)) }
    }).collect();
    let list = List::new(items)
        .block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded)
            .border_style(Style::default().fg(FG_SEP))
            .style(Style::default().bg(BG_SURFACE))
            .title(Span::styled(" ↑↓/Tab select · Esc close ", Style::default().fg(FG_DIM))));
...
fn draw_chat_area(f: &mut Frame, app: &mut App, area: Rect, mode: LayoutMode) {
    ...
    let visible_h = area.height.saturating_sub(2).saturating_sub(if thinking { 1 } else { 0 }) as usize;
    let streaming = matches!(app.state, AppState::Streaming { .. });

    // Cache — invalidated automatically on terminal resize (width mismatch)
    let valid = app.line_cache.as_ref()
    ...
    // ── Visual row counts ─────────────────────────────────────────────────────
    let vrow: Vec<usize> = all.iter().map(|l| {
        let chars: usize = l.spans.iter().map(|s| s.content.chars().count()).sum();
        if content_width == 0 || chars == 0 { 1 }
    ...
    for i in 0..n { vcum[i + 1] = vcum[i] + vrow[i]; }
    ...
    // ── Slice selection ───────────────────────────────────────────────────────
    let start_idx = if n == 0 { 0 } else {
        vcum.partition_point(|&c| c <= scroll_vrow).saturating_sub(1).min(n - 1)
    };
    ...
    if thinking && at_bottom {
        ...
        render.push(Line::from(vec![
            Span::styled("  ◆ ", Style::default().fg(AI_ACCENT).add_modifier(Modifier::BOLD)),
    ...
    let title = if max_scroll > 0 {
        ...
        let ind = if scroll_vrow == 0 { "top".to_string() }
            else if at_bottom { "end".to_string() }
            else { format!("{pct}%") };
    ...
    for msg in messages {
        match msg {
            ...
            ChatMessage::User(text) => {
                ...
                out.push(Line::from(vec![
                    Span::styled("  ▌ ", Style::default().fg(USER_ACCENT).add_modifier(Modifier::BOLD)),
                    Span::styled("You", Style::default().fg(USER_ACCENT).add_modifier(Modifier::BOLD)),
                ]));
            ...
            // ── AI response ───────────────────────────────────────────────
            ChatMessage::Assistant { content, model, tokens } => {
                let model_s: String = if matches!(mode, LayoutMode::Compact|LayoutMode::Tiny) {
                    "AI".into()
                } else { model.chars().take(18).collect() };
                ...
                out.push(Line::from(vec![
                    Span::styled("  ◆ ", Style::default().fg(AI_ACCENT).add_modifier(Modifier::BOLD)),
                    Span::styled(model_s, Style::default().fg(AI_ACCENT).add_modifier(Modifier::BOLD)),
                ...
                for raw in content.lines() {
                    let lines = render_ai_line(raw, &mut in_diff, &mut in_code,
                                               &mut code_lang, content_width, sep_w);
                    out.extend(lines);
                }
                ...
                if in_code {
                    out.push(Line::from(vec![
                        Span::raw("    "),
                        Span::styled("╰".to_string() + &"─".repeat(sep_w.min(40)),
            ...
            ChatMessage::ToolCall { display, ok, .. } => {
                ...
                let ell = if display.chars().count() > max_d { "…" } else { "" };
                out.push(Line::from(vec![
                    Span::styled(icon, Style::default().fg(accent)),
                    Span::styled(format!("{d}{ell}"),
                        Style::default().fg(if ok.is_none() { FG_SUBTEXT } else { FG_DIM })),
                ]));
            }

            // ── Skill result ──────────────────────────────────────────────
            ChatMessage::SkillResult { skill_id, ok, steps, summary } => {
                let (icon, color) = if *ok { ("  ✓ ", C_GREEN) } else { ("  ✗ ", C_RED) };
                out.push(Line::from(vec![
                    Span::styled(icon, Style::default().fg(color).add_modifier(Modifier::BOLD)),
                    Span::styled(
                        format!("Skill '{}' {}  ({} steps)", skill_id,
                            if *ok { "done" } else { "failed" }, steps),
            ...
            // ── Call chain ────────────────────────────────────────────────
            ChatMessage::CallChain { symbol, file, line, incoming, outgoing } => {
                let max_w = content_width.saturating_sub(12);
                out.push(Line::from(vec![
                    Span::styled("  ⌕ ", Style::default().fg(C_CYAN)),
                    Span::styled(symbol.clone(), Style::default().fg(FG_TEXT).add_modifier(Modifier::BOLD)),
                    Span::styled(format!("  {file}:{line}"), Style::default().fg(FG_DIM)),
                ]));
                out.push(Line::from(Span::styled(
                    format!("    ▲ callers ({})", incoming.len()), Style::default().fg(C_YELLOW))));
                if incoming.is_empty() {
                    out.push(Line::from(Span::styled("      (none)", Style::default().fg(FG_DIM))));
                } else {
                    for (j,c) in incoming.iter().enumerate() {
                        let p = if j == incoming.len()-1 { "      └ " } else { "      ├ " };
                        let cd: String = c.chars().take(max_w).collect();
                        out.push(Line::from(Span::styled(format!("{p}{cd}"), Style::default().fg(C_YELLOW))));
                    }
                }
                out.push(Line::from(Span::styled(
                    format!("    ▼ callees ({})", outgoing.len()), Style::default().fg(C_CYAN))));
                if outgoing.is_empty() {
                    out.push(Line::from(Span::styled("      (none)", Style::default().fg(FG_DIM))));
                } else {
                    for (j,c) in outgoing.iter().enumerate() {
                        let p = if j == outgoing.len()-1 { "      └ " } else { "      ├ " };
                        let cd: String = c.chars().take(max_w).collect();
                        out.push(Line::from(Span::styled(format!("{p}{cd}"), Style::default().fg(C_CYAN))));
                    }
                }
                out.push(Line::from(""));
            }
...
fn render_ai_line(
        ...
        if *in_code {
            ...
            *code_lang = String::new();
            return one!(Line::from(vec![
                Span::raw("    "),
                Span::styled("╰".to_string() + &"─".repeat(sep_w.min(40)), Style::default().fg(FG_SEP)),
            ]));
        } else {
            *in_code = true;
            let lang = line.trim_start_matches('`').trim().to_string();
            *code_lang = lang.clone();
            let tag = if lang.is_empty() { " code ".to_string() } else { format!(" {lang} ") };
            let rest_w = sep_w.min(40).saturating_sub(tag.len() + 2);
            return one!(Line::from(vec![
                Span::raw("    "),
                Span::styled("╭─", Style::default().fg(FG_SEP)),
                Span::styled(tag, Style::default().fg(C_YELLOW).add_modifier(Modifier::BOLD)),
    ...
    if *in_diff {
        if line.starts_with('+') && !line.starts_with("+++") {
            return one!(Line::from(vec![
                Span::raw("    "),
                Span::styled("+ ".to_string(), Style::default().fg(C_GREEN).add_modifier(Modifier::BOLD)),
        ...
        if line.starts_with('-') && !line.starts_with("---") {
            return one!(Line::from(vec![
                Span::raw("    "),
                Span::styled("- ".to_string(), Style::default().fg(C_RED).add_modifier(Modifier::BOLD)),
                Span::styled(line[1..].to_string(), Style::default().fg(C_RED)),
            ]));
        }
        if line.starts_with("@@") {
            return one!(Line::from(vec![Span::raw("    "),
                Span::styled(line.to_string(), Style::default().fg(C_CYAN))]));
        }
        if line.starts_with("---") || line.starts_with("+++") || line.starts_with("diff ") {
            return one!(Line::from(vec![Span::raw("    "),
                Span::styled(line.to_string(), Style::default().fg(USER_ACCENT).add_modifier(Modifier::BOLD))]));
        }
    }
    ...
    if line.starts_with("# ") {
        ...
        return one!(Line::from(vec![Span::raw("  "),
            Span::styled(format!(" {padded} "),
                Style::default().fg(BG_BASE).bg(C_YELLOW).add_modifier(Modifier::BOLD))]));
    }
    if line.starts_with("## ") {
        return one!(Line::from(vec![Span::raw("    "),
            Span::styled(line[3..].to_string(),
    ...
    // Bold
    if line.contains("**") { return one!(render_bold(line)); }
    ...
    if line.contains('`') {
        ...
        if parts.len() > 1 {
            ...
            for (j, p) in parts.iter().enumerate() {
                if j % 2 == 0 {
                    if !p.is_empty() { spans.push(Span::raw(p.to_string())); }
                } else {
                    spans.push(Span::styled(p.to_string(),
    ...
    if !first2.is_empty() && first2[0].is_ascii_digit() {
        ...
        if rest.contains(". ") || rest.contains(") ") {
            return one!(Line::from(vec![Span::raw("    "), Span::raw(line.to_string())]));
        }
    }
...
fn render_bold(line: &str) -> Line<'static> {
    ...
    while !rem.is_empty() {
        if let Some(s) = rem.find("**") {
            if s > 0 { spans.push(Span::raw(rem[..s].to_string())); }
            let after = &rem[s+2..];
            if let Some(e) = after.find("**") {
                spans.push(Span::styled(after[..e].to_string(),
                    Style::default().fg(FG_TEXT).add_modifier(Modifier::BOLD)));
                rem = &after[e+2..];
            } else { spans.push(Span::raw(rem[s..].to_string())); break; }
        } else { spans.push(Span::raw(rem.to_string())); break; }
    }
    Line::from(spans)
}
...
fn draw_skill_panel(f: &mut Frame, app: &App, area: Rect) {
    let AppState::SkillRunning { skill_name, current_step, total_steps, step_name, step_status, .. }
        = &app.state else { return; };
    let pct = if *total_steps > 0 { (*current_step * 100 / total_steps) as u16 } else { 0 };
    let split = if area.width >= 80 {
        [Constraint::Percentage(65), Constraint::Percentage(35)]
    } else { [Constraint::Percentage(75), Constraint::Percentage(25)] };
    let inner = Layout::default().direction(Direction::Horizontal)
        .constraints(split).split(area);
    ...
    let nm: String = skill_name.chars().take((inner[0].width as usize).saturating_sub(12).max(4)).collect();
    let gauge = Gauge::default()
        .block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded)
            .border_style(Style::default().fg(FG_BORDER))
            .style(Style::default().bg(BG_SURFACE))
            .title(Span::styled(format!(" {icon} {nm} "), Style::default().fg(gc).add_modifier(Modifier::BOLD))))
        .gauge_style(Style::default().fg(gc).bg(BG_CODE))
        .percent(pct)
        .label(Span::styled(format!("{}/{total_steps}", current_step+1), Style::default().fg(FG_TEXT)));
    ...
    let info = vec![
        ...
        if matches!(step_status, StepStatus::WaitingApproval) {
            Line::from(Span::styled(" ⏸ approve? y/n", Style::default().fg(C_YELLOW)))
        } else if let StepStatus::Failed(e) = step_status {
            Line::from(Span::styled(format!(" ✗ {}", safe_prefix(e, mw.saturating_sub(4))), Style::default().fg(C_RED)))
        } else { Line::from(Span::styled(" working…", Style::default().fg(C_PURPLE))) },
    ];
...
fn draw_approval_modal(f: &mut Frame, message: &str) {
    ...
    let ph = (ml as u16 + 5).clamp(7, 14).min(area.height.saturating_sub(2));
    ...
    for l in message.lines().take(8) {
        let d = if l.chars().count() > iw { format!("  {}…", safe_prefix(l, iw.saturating_sub(1))) }
                else { format!("  {l}") };
        lines.push(Line::from(Span::styled(d, Style::default().fg(FG_TEXT))));
    }
    if ml > 8 { lines.push(Line::from(Span::styled("  …", Style::default().fg(FG_DIM)))); }
    lines.push(Line::from(""));
    lines.push(Line::from(vec![
        Span::styled("  [y] ", Style::default().fg(C_GREEN).add_modifier(Modifier::BOLD)),
        Span::styled("Allow      ", Style::default().fg(FG_SUBTEXT)),
        Span::styled("[n] ", Style::default().fg(C_RED).add_modifier(Modifier::BOLD)),
    ...
    f.render_widget(Paragraph::new(lines)
        .block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded)
            ...
            .style(Style::default().bg(BG_SURFACE))
            .title(Span::styled(" 🔒 Needs approval ", Style::default().fg(C_YELLOW).add_modifier(Modifier::BOLD))))
        .wrap(Wrap { trim: false }),
        pa);
}
...
fn draw_choice_modal(f: &mut Frame, app: &App) {
    let AppState::WaitingChoice {
        ref title, ref options, selected_idx,
        allow_custom, ref custom_input, custom_mode,
    } = app.state else { return };
    ...
    lines.push(Line::from(Span::styled(hint_s, Style::default().fg(FG_DIM))));
...
fn draw_debug_overlay(f: &mut Frame, app: &App) {
    ...
    let rendered: Vec<Line> = visible.iter().map(|line| {
        let (style, marker) = if line.contains(" ERROR ") || line.contains("[ERROR]") || line.contains("⛔") {
            (Style::default().fg(C_RED), "")
        } else if line.contains(" WARNING ") || line.contains("[SOUL] WARNING") || line.contains("⚠") {
            (Style::default().fg(C_YELLOW), "")
    ...
    let block = Block::default()
        ...
        .style(Style::default().bg(BG_BASE))
        .title(Span::styled(" 🪵 Debug Log  (F12:close · F12 again:refresh) ", title_style))
...
fn draw_notification_bar(f: &mut Frame, app: &App, area: Rect) {
    ...
    let Some(ref notif) = app.notification else { return };
    ...
    let ell = if notif.message.chars().count() > max_msg { "…" } else { "" };
...
fn draw_status_bar(f: &mut Frame, app: &App, area: Rect, mode: LayoutMode) {
    ...
    let elapsed_secs = app.request_start
    ...
    let (status_text, status_color, spin) = match &app.state {
        ...
        AppState::Thinking => (
            ...
            C_PURPLE, spinner_frame(app.spinner_tick)),
        AppState::Streaming { tokens_received } => {
            ...
            let recent_tools: Vec<String> = app.messages.iter().rev()
                .filter_map(|m| {
                    if let ChatMessage::ToolCall { display, ok: Some(ok), .. } = m {
                        let icon = if *ok { "✓" } else { "✗" };
        ...
        AppState::SkillRunning { skill_name, current_step, total_steps, .. } => (
            format!("Skill {}  {}/{total_steps}", skill_name.chars().take(12).collect::<String>(), current_step+1),
            C_PURPLE, spinner_frame(app.spinner_tick)),
        AppState::Error(m) => {
    ...
    let ell = if status_text.chars().count() > max_s { "…" } else { "" };

    // Helper for key+label pairs
    let kb = |key: &str, lbl: &str| -> Vec<Span<'static>> { vec![
        Span::styled(key.to_string(), Style::default().fg(C_YELLOW).add_modifier(Modifier::BOLD)),
    ...
    match mode {
        ...
        LayoutMode::Tiny => {
            spans.push(Span::styled(format!("{spin} {sd}"), Style::default().fg(status_color)));
        }
    }

    f.render_widget(
        Paragraph::new(Line::from(spans))
            .style(Style::default().bg(BG_SURFACE).fg(FG_TEXT)),
        area);
}
